...
class MMIODiverger(implicit p: Parameters) extends LLCModule with HasCHIOpcodes {
  ...
  io.in.zip(io.out.cache).zipWithIndex.foreach { case ((in, out), i) =>
    ...
    out.rx.rsp.ready := in.rx.rsp.ready && (!io.out.uncache.rx.rsp.valid || io.out.uncache.rx.rsp.bits.tgtID =/= i.U)
    out.rx.dat.ready := in.rx.dat.ready && (!io.out.uncache.rx.dat.valid || io.out.uncache.rx.dat.bits.tgtID =/= i.U)

    in.tx.req.ready := io.out.uncache.tx.req.ready && reqFromMMIO(in.tx.req.bits) && mmioReqArb.io.chosen === i.U ||
      out.tx.req.ready && !reqFromMMIO(in.tx.req.bits)
    in.tx.dat.ready := io.out.uncache.tx.dat.ready && datFromMMIO(in.tx.dat.bits) && mmioDatArb.io.chosen === i.U ||
      out.tx.dat.ready && !datFromMMIO(in.tx.dat.bits)

    in.rx.rsp.valid := out.rx.rsp.valid || io.out.uncache.rx.rsp.valid && io.out.uncache.rx.rsp.bits.tgtID === i.U
    in.rx.rsp.bits := Mux(io.out.uncache.rx.rsp.valid, io.out.uncache.rx.rsp.bits, out.rx.rsp.bits)
    in.rx.dat.valid := out.rx.dat.valid || io.out.uncache.rx.dat.valid && io.out.uncache.rx.dat.bits.tgtID === i.U
    in.rx.dat.bits := Mux(io.out.uncache.rx.dat.valid, io.out.uncache.rx.dat.bits, out.rx.dat.bits)
    in.rx.snp <> out.rx.snp
  }

  io.out.uncache.tx.req.valid := mmioReqArb.io.out.valid
  ...
  io.out.uncache.rx.rsp.ready := Cat(io.in.zipWithIndex.map { case (in, i) =>
    in.rx.rsp.ready && io.out.uncache.rx.rsp.bits.tgtID === i.U }).orR
...
class MMIOMerger(implicit p: Parameters) extends LLCModule {
  ...
  out.rx.rsp.ready := in.cache.rx.rsp.ready && !rspToMMIO(out.rx.rsp.bits) ||
    in.uncache.rx.rsp.ready && rspToMMIO(out.rx.rsp.bits)
  out.rx.dat.ready := in.cache.rx.dat.ready && !datToMMIO(out.rx.dat.bits) ||
    in.uncache.rx.dat.ready && datToMMIO(out.rx.dat.bits)
